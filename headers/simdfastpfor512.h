 ...
 */
#ifndef SIMDFASTPFOR512_H_
#define SIMDFASTPFOR512_H_
 ...
 *
 */
class SIMDFastPFor512 : public IntegerCODEC {
public:
   ...
   * value should do.
   */
  SIMDFastPFor512(uint32_t ps = 65536)
  ...
  enum {
    ...
    BlockSize = sizeof(__m512i) * bitsPerByte
  };

  static uint32_t *packblockupsimd(const uint32_t *source, uint32_t *out,
                                   const uint32_t bit) {
    for (int k = 0; k < BlockSize; k += sizeof(__m512i) * bitsPerByte) {
      SIMD512_fastpack_32(source, reinterpret_cast<__m512i *>(out), bit);
      out += sizeof(__m512i) / sizeof(uint32_t) * bit;
      source += sizeof(__m512i) * bitsPerByte;
    }
    return out;
  }
  ...
  static const uint32_t *unpackblocksimd(const uint32_t *source, uint32_t *out,
                                         const uint32_t bit) {
    for (int k = 0; k < BlockSize; k += sizeof(__m512i) * bitsPerByte) {
      SIMD512_fastunpack_32(reinterpret_cast<const __m512i *>(source), out, bit);
      source += sizeof(__m512i) / sizeof(uint32_t) * bit;
      out += sizeof(__m512i) * bitsPerByte;
    }
    return source;
  }
  ...
  static const uint32_t *unpackmesimd(const uint32_t *in, STLContainer &out,
    ...
    uint32_t j = 0;
    for (; j + sizeof(__m512i) * bitsPerByte <= size; j += sizeof(__m512i) * bitsPerByte) {
      usimd512unpack(reinterpret_cast<const __m512i *>(in), &out[j], bit);
      in += sizeof(__m512i) / sizeof(uint32_t) * bit;
    }
    for (; j + 31 < size; j += 32) {
  ...
  static uint32_t *packmeupwithoutmasksimd(STLContainer &source, uint32_t *out,
    ...
    *out = size;
    ...
    uint32_t j = 0;
    for (; j + sizeof(__m512i) * bitsPerByte <= size; j += sizeof(__m512i) * bitsPerByte) {
      usimd512packwithoutmask(&source[j], reinterpret_cast<__m512i *>(out), bit);
      out += sizeof(__m512i) / sizeof(uint32_t) * bit;
    }
    for (; j < size; j += 32) {
  ...
  // PD: This vector serves as a temporary storage for the positions of the
  // exceptions within one block. In the original SSE implementation its
  // elements are uint8_t, but for the AVX-512 implementation, this does not
  // suffice, since the block size is 512 and, thus, a position within a block
  // might be greater than 255. Since we only do straightforward ports to newer
  // SIMD extensions, we simply use uint16_t here. This has an impact on some
  // other points of this algorithm, which can be found by following this
  // variable or by looking at the changes in this commit.
  std::vector<uint16_t> bytescontainer;
  ...
  void getBestBFromData(const uint32_t *in, uint16_t &bestb,
                        uint16_t &bestcexcept, uint16_t &maxb) {
    uint32_t freqs[33];
    ...
    uint32_t cexcept = 0;
    bestcexcept = static_cast<uint16_t>(cexcept);
  ...
  void __encodeArray(const uint32_t *in, const size_t length, uint32_t *out,
    ...
    uint16_t *bc = &bytescontainer[0];
    out = padTo64bytes(out);
    assert(!needPaddingTo64Bytes(in));
    for (const uint32_t *const final = in + length; (in + BlockSize <= final);
         in += BlockSize) {
      uint16_t bestb, bestcexcept, maxb;
    ...
    const uint32_t bytescontainersize =
        static_cast<uint32_t>(bc - &bytescontainer[0]) * sizeof(uint16_t);
  ...
  void __decodeArray(const uint32_t *in, size_t &length, uint32_t *out,
    ...
    const uint16_t *bytep = reinterpret_cast<const uint16_t *>(inexcept);
    ...
    in = padTo64bytes(in);
    assert(!needPaddingTo64Bytes(out));
    for (uint32_t run = 0; run < nvalue / BlockSize; ++run, out += BlockSize) {
      const uint16_t b = *bytep++;
      const uint16_t cexcept = *bytep++;
      in = unpackblocksimd(in, out, b);
      if (cexcept > 0) {
        const uint16_t maxbits = *bytep++;
        if (maxbits - b == 1) {
          for (uint32_t k = 0; k < cexcept; ++k) {
            const uint16_t pos = *(bytep++);
  ...
  std::string name() const {
    return std::string("SIMDFastPFor_") + std::to_string(sizeof(__m512i) * bitsPerByte);
  }
};

} // namespace FastPFor

#endif /* SIMDFASTPFOR512_H_ */
